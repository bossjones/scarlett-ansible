...
cd gst-python-1.2.4 && \
./configure --prefix=$VIRT_ROOT PKG_CONFIG_PATH=/usr/local/lib/pkgconfig:$VIRT_ROOT/lib/pkgconfig >> /tmp/scarlett_gst-python_compile.log && \
sudo make > /tmp/scarlett_gst-python_compile.log && \
sudo make install > /tmp/scarlett_gst-python_compile.log && \
cd $MAIN_DIR
